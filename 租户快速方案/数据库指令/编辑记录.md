...
（可选）视图 ID (view_id): 指定一个预定义的 ABS 视图 ID。如果提供，则会返回该视图下的所有记录。不指定视图ID或筛选条件，则会修改表格下的所有数据。

（可选）过滤条件 (filter_by_formula): 提供一个 ABS 公式字符串来筛选记录。不指定视图ID或筛选条件，则会修改表格下的所有数据。

要更新的字段 (updates) (必填): 一个JSON对象，包含了要更新的字段以及字段的值。例： { "工号": "1", "状态": "完成"}
...
记录必须存在: 目标表中应该要存在记录。尝试更新不存在的记录会导致该条记录更新失败或无反应。

字段必须存在: updates 中提供的字段名必须在目标表中存在。尝试更新不存在的字段可能会被忽略或导致错误。

部分更新:  updates 中只需提供需要修改的字段及其新值即可，未提供的字段将保持不变。
...
（可选）指定一个预定义的 ABS 视图 ID。如果提供，则会返回该视图下的所有记录。不指定视图ID或筛选条件，则会修改表格下的所有数据。
...
逻辑符语法：等于(eq)，不等于(neq)，大于(gt)，大于等于(gte)，小于(lt)，小于等于(lte)，模糊搜索-匹配/不匹配(like/nlike)，为空(blank)，不为空(notblank)

多个筛选条件用~or(或),~and(与)连接，举例："(名字,eq,李四)~or(名字,eq,张三)"

不指定视图ID或筛选条件，则会修改表格下的所有数据。

否

updates

 一个JSON对象，包含了要更新的字段以及字段的值。例： { "工号": "1", "状态": "完成"} 
...
注意：如果不指定视图ID或筛选条件，则会修改表格下的所有数据。
